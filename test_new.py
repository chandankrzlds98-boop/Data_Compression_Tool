...
def compress_video(input_path: str, output_path: str, compression_percentage: int,
    ...
    """
    Map main compression percentage (0-100) to an H.264 CRF value.
    Higher percentage -> higher CRF -> smaller file.
    Audio track parameters are derived from voice_compression_percentage.
    """
    crf = int(18 + (compression_percentage / 100) * 22)
    ...
    cmd =[
            ...
            "-c:v", "libx264",
            "-crf", str(crf),
            "-preset", "veryfast",  # Options: 'ultrafast', 'superfast', or 'veryfast'
            "-threads", "0",        # Auto-detects and uses all available CPU cores
            "-c:a", "aac",
            "-b:a", audio_bitrate,
]
    ...
    # Downscale resolution as main compression percentage increases
...
z
  ...
  .stButton>button {
    width: 100%;
    background: linear-gradient(135deg, var(--signal-blue), var(--navy-1000)) !important;
    color: #ffffff !important;
